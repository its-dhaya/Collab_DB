...
def process_command(command):
    ...
    # Split the WHERE clause into condition_field and condition_value
        ...
        if len(tokens) < 3:
            return "Syntax error. Usage: DELETE FROM table_name; OR DELETE FROM table_name WHERE condition;"

    # Ensure "FROM" is in the correct position
    ...
    # Case 1: Delete all records from the table (NO WHERE CLAUSE)
    if len(tokens) == 3:
        database[table_name] = []  # Clear all records but keep the table
        save_db()
        return f"All records deleted from '{table_name}'."

    # Case 2: Delete specific records using WHERE condition
    if len(tokens) > 3 and tokens[3].lower() == "where":
        if len(tokens) < 6 or "=" not in " ".join(tokens[4:]):
            return "Syntax error in WHERE clause. Expected format: DELETE FROM table_name WHERE field = value;"
    ...
    # If extra tokens but no "WHERE" keyword, it's an invalid command
    return "Syntax error. Use: DELETE FROM table_name; OR DELETE FROM table_name WHERE condition;"
